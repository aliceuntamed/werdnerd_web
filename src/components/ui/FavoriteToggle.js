...
export const FavoriteToggle = ({ isFavorite, onToggle, size = 28, label = "favorite", className = "", }) => {
    // If controlled: internal state mirrors parent
    // If uncontrolled: internal state manages itself
    const [internalFav, setInternalFav] = useState(isFavorite ?? false);
    ...
    return (<button onClick={handleToggle} aria-pressed={internalFav} className={`
      ...
      `}>
      {/* Outline (off) */}
      {!internalFav && (<Heart size={size} strokeWidth={1.75} className="
