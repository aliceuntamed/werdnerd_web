...
export function Tag({ label, index = 0, onClick, className = "", active = false, size = "md", }) {
  ...
  `;
    ...
    if (onClick) {
        return (<button type="button" onClick={onClick} aria-pressed={active} className={`${baseClasses} cursor-pointer ${className}`} style={style}>
        {label}
      </button>);
    }
    return (<span className={`${baseClasses} ${className}`} style={style}>
      {label}
    </span>);
}
