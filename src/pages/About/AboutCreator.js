...
export function AboutCreator() {
    ...
    return (<section className="w-full bg-bg-main text-text-primary font-body px-6 py-20">
      <div className="max-w-5xl mx-auto grid md:grid-cols-2 gap-16">
        {/* LEFT SIDE — Featured Image + Quote */}
        <div className="flex flex-col gap-6">
          <div className="relative overflow-hidden rounded-card shadow-soft-elevated">
            <img src="https://images.pexels.com/photos/9572404/pexels-photo-9572404.jpeg?auto=compress&cs=tinysrgb&w=200" alt="Creative workspace with typewriter and books" className="w-full h-80 object-cover opacity-90"/>
        ...
        <div className="flex flex-col gap-8">
          ...
          <div className="flex gap-4 mt-4">
            <Link to="/" className="rounded-full px-5 py-2 bg-bg-elevated border border-border-subtle text-sm font-medium hover:bg-bg-subtle transition-colors">
              <span className="bg-chrome-horizontal bg-clip-text text-transparent">
                Explore the Vault
              </span>
            </Link>

            <button onClick={() => setOpen(true)} className="rounded-full px-5 py-2 border border-border-subtle text-sm hover:bg-bg-subtle transition-colors">
              Say Hello
            </button>
          </div>
        </div>
      ...
      {/* Modal */}
      {open && (<div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50" onClick={() => setOpen(false)}>
          <div className="bg-bg-elevated border border-border-subtle rounded-card p-8 w-full max-w-md shadow-soft-elevated" onClick={(e) => e.stopPropagation()}>
            ...
            <form className="flex flex-col gap-4">
              <div className="flex flex-col gap-1">
                ...
                <input type="text" className="bg-bg-subtle border border-border-subtle rounded-card px-3 py-2 text-sm" placeholder="Your name..."/>
              </div>

              <div className="flex flex-col gap-1">
                <label className="text-sm">Email</label>
                <input type="email" className="bg-bg-subtle border border-border-subtle rounded-card px-3 py-2 text-sm" placeholder="you@email.com"/>
              </div>

              <div className="flex flex-col gap-1">
                <label className="text-sm">Message</label>
                <textarea className="bg-bg-subtle border border-border-subtle rounded-card px-3 py-2 text-sm" placeholder="Tell me something unusual..." rows={4}></textarea>
              </div>

              <div className="flex gap-4 mt-2">
                <button type="submit" className="rounded-full px-5 py-2 bg-bg-elevated border border-border-subtle text-sm hover:bg-bg-subtle transition-colors">
                  <span className="bg-chrome-horizontal bg-clip-text text-transparent">
                    Send Werd
                  </span>
                </button>
