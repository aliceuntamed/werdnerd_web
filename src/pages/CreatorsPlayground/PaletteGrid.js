    ...
    return (<section className="px-6 py-20 bg-bg-main text-text-primary">
      <div className="max-w-5xl mx-auto flex flex-col gap-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {palettes.map((p, i) => (<div key={i} className="bg-bg-elevated border border-border-subtle rounded-card p-4 flex flex-col gap-3 shadow-soft-elevated">
              <div className={`h-24 w-full rounded-card ${p.outline ? "border border-accent-pink bg-bg-subtle" : ""}`} style={!p.outline ? { background: p.hex } : {}}></div>
