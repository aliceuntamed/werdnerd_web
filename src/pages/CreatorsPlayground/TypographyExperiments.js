    ...
    return (<section className="px-6 py-20 bg-bg-main text-text-primary">
      <div className="max-w-5xl mx-auto flex flex-col gap-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {experiments.map((item, i) => (<div key={i} className="bg-bg-elevated border border-border-subtle rounded-card p-6 shadow-soft-elevated flex flex-col gap-3">
              ...
              {item.badge && (<span className="text-xs px-2 py-1 rounded-full bg-bg-subtle border border-border-subtle w-fit">
                  {item.badge}
                </span>)}
            </div>))}
        </div>
