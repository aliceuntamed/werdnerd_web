...
export default function BogglePage() {
    ...
    return (<div className="min-h-screen pt-20 pb-32 px-6 relative">
      ...
      <div className="relative z-10 flex flex-col items-center gap-10">
        ...
        <BoggleBoard board={engine.board} selected={selected} onSelect={handleTileClick}/>

        {/* Submit + New Board buttons */}
        <div className="flex gap-4 mt-6">
          <button onClick={submitWord} className="
          ...
          <button onClick={newBoard} className="
              px-5 py-2 rounded-lg font-heading text-white
              border border-white/20
            " style={{
            background: "linear-gradient(135deg, #e5e7eb, #9bbcff, #c084fc)",
        ...
        <div className="mt-4 text-white/70 font-body text-sm">
          {foundWords.length > 0 && (<div className="text-center">
              <p className="mb-2">Found Words:</p>
              <div className="flex flex-wrap gap-2 justify-center">
                {foundWords.map((w) => (<span key={w} className="
