    ...
    return (<div className="max-w-4xl mx-auto">
      ...
      <div className="relative rounded-2xl border border-white/10 bg-white/[0.04] backdrop-blur-md p-8 md:p-12 text-center overflow-hidden">
        ...
        <div className="min-h-[12rem] flex flex-col items-center justify-center mb-8">
          {!spun && !loading && (<p className="font-body text-white/30 italic text-lg">
          ...
          {!loading && werd && (<div className="flex flex-col items-center gap-3 animate-in fade-in slide-in-from-bottom-4 duration-500">
              <h3 className="font-heading text-5xl md:text-6xl chrome-gradient-text leading-none">
                {werd.werd}
              </h3>

              {werd.pronunciation && (<p className="font-body text-white/40 italic text-lg">
                  /{werd.pronunciation}/
                </p>)}

              {werd.part_of_speech && (<span className="text-xs tracking-widest uppercase text-white/30 font-body border border-white/10 px-3 py-1 rounded-full">
                  {werd.part_of_speech}
                </span>)}
              ...
              <Link to={`/vault?search=${encodeURIComponent(werd.werd)}`} className="mt-2 text-sm font-body text-white/40 hover:text-white transition-colors underline underline-offset-4 decoration-white/15 hover:decoration-white">
                See full entry →
              </Link>
            </div>)}
        </div>

        <button onClick={spin} disabled={loading} className="inline-flex items-center gap-3 px-8 py-4 rounded-xl font-heading text-base text-black bg-chrome-horizontal shadow-lg hover:opacity-90 active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed">
